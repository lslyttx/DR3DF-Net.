...
from My_Model_capa import Base_Model, Discriminator
...
pre_model = torch.nn.DataParallel(pre_model, device_ids=device_ids)
DNet = DNet.to(device)
...
for epoch in range(start_epoch, train_epoch):
    ...
    with tqdm(total=len(train_loader)) as t:
        for (hazy, clean) in train_loader:
            ...
            total_loss = smooth_loss_l1 + 0.01 * perceptual_loss + 0.0005 * adversarial_loss + 0.5 * msssim_loss_ + 1.5 * smooth_loss_l1_img
