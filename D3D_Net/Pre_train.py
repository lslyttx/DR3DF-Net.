...
from My_Model_L import Base_Model, Discriminator
...
parser.add_argument('--model_save_dir', type=str, default='./Pre_Train_Model')
...
for epoch in range(start_epoch, train_epoch):
    ...
    with tqdm(total=len(train_loader)) as t:
        for (hazy, clean) in train_loader:
            ...
            adversarial_loss = torch.mean(1 - img_out)
            ...
            total_loss = smooth_loss_l1 + 0.01 * perceptual_loss + 0.0005 * adversarial_loss + 0.5 * msssim_loss_ + 1.5 * smooth_loss_l1_img  # 0.01 * perceptual_loss_img + 0.0005 * adversarial_loss_img + 0.5 * msssim_loss_img +
            ...
            t.set_description(
                "===> Epoch[{}] :  total_loss: {:.2f}  D_loss: {:.2f} ".format(
                    epoch, total_loss.item(), D_loss.item(),
    ...
    if tag in []:
        if epoch >= 30:
            sep = 1
    elif tag in ['thin', 'thick', 'moderation', 'RICE1', 'RICE2', 'RSID','NID','moderation_pt','RICE1_ALL','RICE2_ALL']:
        if epoch >= 50:
            sep = 1
    else:
        if epoch >= 500:
